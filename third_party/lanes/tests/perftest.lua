...
--      -single: runs using just a single CPU lanes (or 'n' laness if given)
...
local SINGLE= 0     -- laness to use (0 / 1..n) 
...
if PLAIN then
    ...
    if SINGLE > 0 then
        io.stderr:write( (tonumber(SINGLE) and SINGLE or 1) .. " lanes(s):\n" )
        lanes.set_singlethreaded(SINGLE)    -- limit to N laness (just OS X)
    else
        io.stderr:write( "Multi lanes:\n" )
    end
