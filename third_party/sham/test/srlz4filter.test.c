...
static void
srlz4filter_compress_decompress(stc *cx srunused)
{
	sra a;
	sr_aopen(&a, &sr_stda);
	sr r;
	...
	srfilter f;
	t( sr_filterinit(&f, &sr_lz4filter, &r, SR_FINPUT) == 0 );
	...
	t( sr_filterinit(&f, &sr_lz4filter, &r, SR_FOUTPUT) == 0 );
...
stgroup *srlz4filter_group(void)
{
	stgroup *group = st_group("srlz4filter");
	st_groupadd(group, st_test("compress_decompress", srlz4filter_compress_decompress));
	return group;
}
