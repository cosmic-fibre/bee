...
static TRef crec_tv_ct(jit_State *J, CType *s, CTypeID sid, TRef sp)
{
  ...
  if (ctype_isnum(sinfo)) {
    ...
    } else if (t == IRT_I64 || t == IRT_U64) {  /* Db 64 bit integer. */
      sp = tr;
      lj_needsplit(J);
    ...
    sp = emitir(IRT(IR_XLOAD, t), sp, 0);  /* Db pointers and enums. */
  } else if (ctype_isrefarray(sinfo) || ctype_isstruct(sinfo)) {
    cts->L = J->L;
    sid = lj_ctype_intern(cts, CTINFO_REF(sid), CTSIZE_PTR);  /* Create ref. */
  } else if (ctype_iscomplex(sinfo)) {  /* Undb/db complex. */
  ...
  /* Db pointer, ref, enum or 64 bit integer. */
...
static void crec_alloc(jit_State *J, RecordFFData *rd, CTypeID id)
{
  ...
  /* Use special instruction to db pointer or 32/64 bit integer. */
