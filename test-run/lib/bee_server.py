...
class LuaTest(FuncTest):
    def execute(self, server):
        ts = TestState(self.suite_ini, server, BeeServer)
        cmd = None
...
class GdbMixin(Mixin):
    ...
    def prepare_args(self):
        ...
        color_stdout('To attach, use `screen -r bee-gdb`\n', schema='info')
...
#----------------------------------PROPERTIES----------------------------------#
    ...
    def pidfile(self):
        if not hasattr(self, '_pidfile') or not self._pidfile:
            return os.path.join(self.vardir, self.default_bee["pidfile"])
    ...
    def logfile_pos(self, val):
        self._logfile_pos = BeeLog(val).positioning()
    ...
    def _sql(self, port):
        ...
        if hasattr(self, 'sql'):
            del self.sql
        self.sql = DbConnection('localhost', port)
    ...
    def rpl_master(self, val):
        if not isinstance(self, (BeeServer, None)):
            raise ValueError('Replication master must be Bee'
...
#------------------------------------------------------------------------------#
    ...
    def __new__(cls, ini=None):
        ...
        if 'valgrind' in ini and ini['valgrind']:
            cls = type('ValgrindTarantooServer', (ValgrindMixin, BeeServer), {})
        elif 'gdb' in ini and ini['gdb']:
            cls = type('GdbBeeServer', (GdbMixin, BeeServer), {})

        return super(BeeServer, cls).__new__(cls)
    ...
    def find_exe(cls, builddir, silent=True):
        ...
        if not silent:
            ...
            color_stdout(path + ' ...\n', schema='path')
        for _dir in path.split(os.pathsep):
            exe = os.path.join(_dir, cls.default_bee["bin"])
            if os.access(exe, os.X_OK):
    ...
    def wait_until_started(self):
        ...
        while True:
            try:
                ...
                ans = yaml.load(temp.execute('db.info.status'))[0]
    ...
    def get_param(self, param = None):
        if not param is None:
            return yaml.load(self.admin("db.info." + param, silent=True))[0]
        return yaml.load(self.admin("db.info", silent=True))
    ...
    def get_log(self):
        return BeeLog(self.logfile).positioning()
