...
ffi.C.db_set_log_level(4)

local CONSOLE_SOCKET = '/tmp/bee-test-console.sock'
local IPROTO_SOCKET = '/tmp/bee-test-iproto.sock'
...
test:ok(string.match(handshake, '^Bee .*console') ~= nil, 'Handshake')
...
-- create user
db.schema.user.create('test', { password = 'pass' })
...
db.schema.user.grant('test', 'execute', 'universe')
