...
admin("db.schema.user.grant('test', 'execute,read,write', 'universe')")
...
admin("space = db.schema.space.create('tweedledum')")
...
admin("t = db.tuple.new('tuple', {1, 2, 3}, { k1 = 'v', k2 = 'v2'})")
test("t")
test("t, t, t")
test("{t}")
test("{t, t, t}")
...
test('space:select{}')
test('db.begin(), space:auto_increment({"failed"}), db.rollback()')
test('space:select{}')
