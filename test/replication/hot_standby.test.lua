...
db.schema.func.create('_set_pri_lsn')
db.schema.user.grant('guest', 'execute', 'function', '_set_pri_lsn')
...
fiber = require('fiber');
while db.info.server.id == 0 do fiber.sleep(0.01) end;
while db.space['_priv']:len() < 1 do fiber.sleep(0.001) end;
do
    local pri_id = ''
    local begin_lsn = 0
    ...
    function _insert(_begin, _end)
        ...
        for i = _begin, _end do
            table.insert(a, db.space.tweedledum:insert{i, 'the tuple '..i})
        end
        return a
    end
    ...
    function _select(_begin, _end)
        local a = {}
        for i = _begin, _end do
            local tuple = db.space.tweedledum:get{i}
            if tuple ~= nil then
...
REPLICA ~= nil
a = (require 'net.db'):new(REPLICA.host, REPLICA.service)
a:call('_set_pri_lsn', db.info.server.id, db.info.server.lsn)
a:close()
...
--# stop server default
--# set connection hot_standby
while db.info.status ~= 'running' do fiber.sleep(0.001) end
--# set connection replica
...
HOT_STANDBY ~= nil
a = (require 'net.db'):new(HOT_STANDBY.host, HOT_STANDBY.service)
a:call('_set_pri_lsn', db.info.server.id, db.info.server.lsn)
a:close()
