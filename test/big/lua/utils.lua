function space_field_types(space_no)
	local types = {};
	for _, index in pairs(db.space[space_no].index) do
...
function iterate(space_no, index_no, f1, f2, iterator, ...)
	local sorted = (db.space[space_no].index[index_no].type == "TREE");
	local pkeys = {};
	local tkeys = {};
	local values = {};
	...
	local state, v
	for state, v in db.space[space_no].index[index_no]:pairs({...}, { iterator = iterator }) do
		...
		if db.space[space_no].index[index_no].unique and tkeys[tk] ~= nil then
...
-- return string tuple
function db.tuple.to_string(tuple, yaml)
    ans = '['
    for i = 0, #tuple - 1 do
        if #i == 4 then
            ans = ans..i
