...
s0 = db.schema.space.create('tweedledum')
...
-- https://bugs.launchpad.net/bee/+bug/1072624
s0:insert{'xxxxxxx'}
s0:insert{''}
s0:insert{'12'}

s1 = db.schema.space.create('tweedledee')
i1 = s1:create_index('primary', { type = 'tree', parts = {1, 'str'}, unique = true })

s2 = db.schema.space.create('alice')
...
function crossjoin(space0, space1, limit)
    ...
    for state, v0 in space0:pairs() do
        for state, v1 in space1:pairs() do
            ...
            table.insert(result, db.tuple.new(newtuple))
            limit = limit - 1
        end
    end
    return result
...
s1:insert{'abcdc'}
s1:insert{'abcdc_'}
db.sort(s1.index['primary']:select('abcdb', { limit = 3, iterator = 'GE' }))
s1:drop()
s1 = nil
s2:drop()
s2 = nil
