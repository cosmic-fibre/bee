...
$(function() {
	...
	var download_drop_db = '.b-download-list-item-drop';
	if ($(drop_item)) {
	...
	if ($(download_drop_item)) {
		...
		for (var i=0; i<download_drop_items.length; i++) {
			if($(download_drop_items[i]).parents(download_drop_item_parent).find(download_drop_db).length!=0) {
				$(download_drop_items[i]).addClass(close);
			}
		};
	}
	$(download_drop_item).click(function() {
		if($(this).parents(download_drop_item_parent).find(download_drop_db).length!=0) {
			if ($(this).hasClass(open)) {
				$(this).removeClass(open);
				$(this).parents(download_drop_item_parent).find(download_drop_db).removeClass(open);
			} else {
				$(this).addClass(open);
				$(this).parents(download_drop_item_parent).find(download_drop_db).addClass(open);
			}
		}
		return false;
	});
