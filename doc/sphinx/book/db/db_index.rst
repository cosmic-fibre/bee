...
The ``db.index`` package provides read-only access for index definitions and
index keys. Indexes are contained in :samp:`db.space.{space-name}.index` array within
...
``db.index`` in the storage engine.
...
.. class:: index_object
    ...
    .. data:: parts
        ...
        .. code-block:: lua
            ...
            bee> db.space.tester.index.primary
            ---
            unique: true
            parts:
            0:
    ...
    .. function:: pairs(bitset-value | field-value..., iterator-type)
        ...
        to know the principles of the Bee transaction processing subsystem. An
        iterator in Bee does not own a consistent read view. Instead, each
        ...
        .. container:: table
            ...
            **TREE iterator types**
            ...
            +===============+===========+=============================================+
            | db.index.ALL | none      | Iterate over all tuples in an index. Tuples |
            | or 'ALL'      |           | are returned in ascending order of the key. |
            +---------------+-----------+---------------------------------------------+
            | db.index.EQ  | field     | Equality iterator: iterate over all tuples  |
            ...
            +---------------+-----------+---------------------------------------------+
            | db.index.GT  | field     | Keys match if key values are greater than   |
            ...
            +---------------+-----------+---------------------------------------------+
            | db.index.REQ | field     | Reverse equality iterator. Matching is      |
            | or 'REQ'      | values    | determined in the same way as for           |
            |               |           | ``db.index.EQ``, but, if there are multiple|
            |               |           | matches, then tuples are returned in        |
            |               |           | descending order by key,                    |
            +---------------+-----------+---------------------------------------------+
            | db.index.GE  | field     | Keys match if key values are greater than   |
            ...
            +---------------+-----------+---------------------------------------------+
            | db.index.LT  | field     | Keys match if key values are less than      |
            ...
            |               |           | key in the index.                           |
            +---------------+-----------+---------------------------------------------+
            | db.index.LE  | field     | Keys match if key values are less than or   |
            ...
            **HASH iterator types**
            ...
            +---------------+-----------+---------------------------------------------+
            | db.index.GT  | field     | Keys match if hashed key values are greater |
            ...
            **BITSET iterator types**
            ...
            |                            |           | separated by commas.                        |
            +----------------------------+-----------+---------------------------------------------+
            | db.index.BITS_ALL_SET     | field     | Keys match if all of the bits specified in  |
            |                            | values    | 'bit mask' are set.                         |
            +----------------------------+-----------+---------------------------------------------+
            | db.index.BITS_ANY_SET     | field     | Keys match if any of the bits specified in  |
            |                            | values    | 'bit mask' is set.                          |
            +----------------------------+-----------+---------------------------------------------+
            | db.index.BITS_ALL_NOT_SET | field     | Keys match if none of the bits specified in |
            |                            | values    | 'bit mask' is set.                          |
            +----------------------------+-----------+---------------------------------------------+
            ...
            **RTREE iterator types**
            ...
            | Type               | Arguments | Description                                 |
            +====================+===========+=============================================+
            | db.index.ALL      | none      | All keys match. Tuples are returned in      |
            | or 'ALL'           |           | ascending order of the primary key.         |
            +--------------------+-----------+---------------------------------------------+
            | db.index.EQ       | field     | Keys match if the rectangle defined by the  |
        ...
        .. code-block:: lua
            ...
            bee> s = db.schema.space.create('space17')
            ---
            ...
            bee> s:create_index('primary', {parts = {1, 'STR', 2, 'STR'}})
            ---
            ...
            bee> s:insert{'C', 'C'}
            ---
            ...
            bee> function example()
                     >   for _, tuple in
                     >   s.index.primary:pairs(nil, {iterator = db.index.ALL}) do
                     >     print(tuple)
                     >   end
                     > end!
            ---
    ...
    .. function:: select(key, options)
        ...
        This is is an alternative to :func:`db.space...select() <space_object.select>`
        ...
        .. code-block:: lua
            ...
            db.space.tester:create_index('secondary', {type = 'tree', unique = false, parts = {2, 'STR'}})
            db.space.tester:insert{1,'X','Row with field[2]=X'}
            db.space.tester:insert{2,'Y','Row with field[2]=Y'}
            db.space.tester:insert{3,'Z','Row with field[2]=Z'}
            db.space.tester.index.secondary:select({'X'}, {iterator = 'GT', limit = 1000})
        ...
        .. NOTE::
            ...
            above, ``db.space.tester:select({1}, {iterator = 'GT'})`` would have
        ...
        .. NOTE::
            ...
            ``db.space.tester:select{}`` will select every tuple in the tester
            space via the first (primary-key) index.

        .. NOTE::

            ``db.space.space-name.index.index-name:select(...)[1]``. can be
            replaced by ``db.space.space-name.index.index-name:get(...)``.
        ...
        .. code-block:: lua
            ...
            s = db.schema.space.create('space_with_bitset')
            ...
            s.index.bitset_index:select(0x02,{iterator=db.index.EQ})
            s.index.bitset_index:select(0x02,{iterator=db.index.BITS_ANY_SET})
            s.index.bitset_index:select(0x02,{iterator=db.index.BITS_ALL_SET})
            s.index.bitset_index:select(0x02,{iterator=db.index.BITS_ALL_NOT_SET})
            ...
    ...
    .. function:: min([key-value])
        ...
        .. code-block:: lua
            ...
            bee> db.space.tester.index.primary:min()
            ---
    ...
    .. function:: max([key-value])
        ...
        .. code-block:: lua
            ...
            bee> db.space.tester.index.primary:max()
            ---
    ...
    .. function:: random(random-value)
        ...
        .. code-block:: lua
            ...
            bee> db.space.tester.index.secondary:random(1)
            ---
    ...
    .. function:: count(key-value, options)
        ...
        .. code-block:: lua
            ...
            bee> db.space.tester.index.primary:count(999)
            ---
            - 0
            ...
            bee> db.space.tester.index.primary:count('Alpha!', { iterator = 'LE' })
            ---
            - 1
            ...
    ...
    .. function:: alter({options})
        ...
        .. code-block:: lua
            ...
            bee> db.space.space55.index.primary:alter({type = 'HASH'})
            ---
            ...
    ...
    .. function:: drop()
        ...
        .. code-block:: lua
            ...
            bee> db.space.space55.index.primary:drop()
            ---
            ...
    ...
    .. function:: rename(index-name)
        ...
        .. code-block:: lua
            ...
            bee> db.space.space55.index.primary:rename('secondary')
            ---
            ...
...
===========================================================
                         Example showing use of the db functions
===========================================================
...
The function uses Bee db functions
:func:`db.space...select <space_object.select>`,
:func:`db.space...replace <space_object.replace>`, :func:`fiber.time`,
...
.. code-block:: lua
    ...
    function example()
      ...
      table_of_selected_tuples = db.space.tester:select{1000}
      if table_of_selected_tuples ~= nil then
        if table_of_selected_tuples[1] ~= nil then
          if #table_of_selected_tuples[1] == 3 then
            db.error({code=1, reason='This tuple already has 3 fields'})
          end
        end
      end
      replaced_tuple = db.space.tester:replace
...
.. code-block:: lua
    ...
    bee> db.space.tester:delete(1000)
    ---
...
===========================================================
             Package `db.index` with index type = RTREE for spatial searches
===========================================================

The :mod:`db.index` package may be used for spatial searches if the index type
...
.. code-block:: lua
    ...
    s = db.schema.create_space('rectangles')
...
.. _R tree index quick start and usage: https://github.com/bee/bee/wiki/R-tree-index-quick-start-and-usage
